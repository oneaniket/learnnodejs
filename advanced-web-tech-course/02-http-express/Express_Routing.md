...
For larger applications, routes should be separated into different
files.
...
The `next()` function passes control to the next middleware or route
handler.
...
Express routing connects an HTTP method and URL path to a handler
function.
...
-   Keep routes, controllers, and business logic separate in larger
    projects.

------------------------------------------------------------------------

# Outputs for the Code Examples
...
## Multiple Route Parameters (/users/10/orders/55)
...
## Query Parameters (/users?role=admin&page=2)
